...
```powershell
.venv\Scripts\python.exe prl_draft\scripts\build_figures.py
```

The script verifies the primary raw-data digest before rendering and writes:

- `prl_draft/figures/full_sphere_matched.pdf` and `.png`;
- `prl_draft/figures/matched_field_scaling.pdf` and `.png`;
- `prl_draft/figures/figure_manifest.json`.
...
- `prl_draft/main.pdf`: 4 pages, double column, four main-text figures.
- `prl_draft/supplement.pdf`: 5 pages, single column, one supplemental figure.
...
```powershell
.venv\Scripts\python.exe -m py_compile prl_draft\scripts\build_figures.py
.venv\Scripts\python.exe prl_draft\scripts\build_figures.py
```
