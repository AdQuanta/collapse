...
| `core/analysis.py` and `core/relative_evolution_pencil.py` | Canonical production definitions and stable homogeneous generalized eigensolver. Code explicitly documents `U10 v=lambda U00 v`, `theta=2 atan|lambda|`, and the antipodal second branch. | Older paths in `analysis.py` still expose direct solves/pseudoinverse fallbacks; publication claims should use the homogeneous solver and residual diagnostics. |
