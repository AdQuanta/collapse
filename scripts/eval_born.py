"""Fixed Verifier / Evaluator for Born-Similarity Optimization at N=10 (configurable).
...
# Fixed default long-time collection (6 points between 100.0 and 1000.0, completing in < 10s at N=10)
DEFAULT_LONG_TIMES = tuple(np.geomspace(100.0, 1000.0, 6))


def build_candidate_hamiltonian(config: dict[str, Any], n_pixel: int = 10) -> SinglePixelHamiltonianQuSpin:
    ...
    filtered_kwargs.setdefault("use_symmetry", False)
    return SinglePixelHamiltonianQuSpin(N_pixel=n_pixel, **filtered_kwargs)


def evaluate_born_score(
    config: dict[str, Any],
    n_pixel: int = 10,
    ...
    # 2. Diagonalize once (using LAPACK eigh on dense Hermitian operator)
    try:
        energies, vectors = np.linalg.eigh(h_dense)
    ...
    for t in times:
        ...
        # Solve M = U00^(-1) U10 for well-conditioned U00, with QZ pencil fallback
        cond = float(np.linalg.cond(u00))
        if cond < 1.0e6:
            spec = diagonalize_relative_evolution(u00, u10, solver="solve")
            radii = spec.radii
        else:
            spectrum = generalized_relative_evolution_spectrum(u00, u10, compute_left_eigenvectors=False)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate Born-similarity at fixed N for a candidate Hamiltonian.")
    parser.add_argument("--candidate", type=Path, required=True, help="Path to candidate JSON config.")
    parser.add_argument("--n-pixel", type=int, default=10, help="Number of detector spins (default: 10).")
    ...
    print(f"[*] Evaluating candidate {args.candidate.name} at N={args.n_pixel} across {len(DEFAULT_LONG_TIMES)} long times...")
    result = evaluate_born_score(config_data, n_pixel=args.n_pixel, max_epsilon=args.max_epsilon)
