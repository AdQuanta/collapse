"""Build one far-from-Born approval sample for the hz0=0.1 detector-resonance atlas."""
...
def render_case_2x3(
    ...
    *,
    ...
    figure.suptitle(
        rf"Non-Born example: {family}, dynamics $N={record.dynamics_n}$, {record.config_id}, "
...
DEFAULT_OUTPUT = ROOT / "reports/hz0_nonborn_sample_2x3_2026-08-01"


def select_worst_case() -> tuple[CaseRecord, float, Path]:
    ...
    return min(candidates, key=lambda item: (item[0].s_born, item[0].config_id))


def main() -> None:
    record, hz0, source = select_worst_case()
    ...
    manifest = {
        "created": "2026-08-01",
        "selection": "minimum finite S_born among successful cases in the five downloaded hz0=0.1 campaigns",
