...
**Part C — Family Q as a spectral problem.** By T5 the zero-field rings reduce to the eigenphase law of W = e^{it(H_D+gS)}·e^{−it(H_D−gS)}, S = ΣX_i. At weak coupling the eigenphases are ≈ 2g times the eigenvalues of the dephased Schrieffer–Wolff generator K = Σ_{a≠b} S_ab/(E_a−E_b)·e^{iφ_ab}|a⟩⟨b|. Derive the eigenvalue density of K for an XXZ ring, or for a random-matrix model of it, and ask when its folded law has the Born form (1+cos θ)·even. (Detector-only echo spectra reproduce the repository's N = 12 ratio RMSE, 0.132 against 0.129, in three minutes on a laptop; at that size 45 % of the eigenphases are within 0.1 rad of zero.) This is a random-matrix question with no measurement content in it, and it is where the repository's best numbers live.
