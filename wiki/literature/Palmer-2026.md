...
While the `unitary-collapse` project defines collapsible states as the projective roots of exact dual homogeneous pencils $(U_{10} + \lambda U_{11})|D\rangle=0$ and $(U_{00} + \lambda U_{01})|D\rangle=0$ (a dynamical unitary origin), RaQM suggests that any "physical" state must be rational (an arithmetic origin). Under SPEC v1.0, all candidate Born-like root regimes require fresh objective verification, and all paper-readiness gates are `INCOMPLETE`.
