...
Both Griffiths' consistent histories and `unitary-collapse` seek an observer-free, unitary resolution to quantum measurement. While Griffiths allows an infinite multiplicity of incompatible consistent frameworks (leaving open which framework is physically realized), `unitary-collapse` hypothesizes that the detector Hamiltonian's exact dual outcome pencils $(U_{10}, -U_{11})$ and $(U_{00}, -U_{01})$ select a unique, stable preferred measurement basis. Under SPEC v1.0, this basis-uniqueness requirement is an unverified gate and remains `INCOMPLETE`.
