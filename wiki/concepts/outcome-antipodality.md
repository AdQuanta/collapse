...
3. **The reflected polar scoring in the codebase is exact, not a surrogate.** Marginalizing over azimuth turns the antipodal map into $\theta\mapsto\pi-\theta$, so $\rho_1^{(\theta)}(\theta)=\rho_0^{(\theta)}(\pi-\theta)$, which is precisely the condition [[born-like-points]] names before a single reflected histogram may be read as the SPEC weak ratio. The `born_ratio_from_theta(theta, pi - theta)` scoring used throughout the search scripts therefore computes the weak criterion itself — provided the histogrammed roots are an outcome set. Those scripts in fact histogram the fixed-input pencil, which carries the same polar law for every time-reversal-invariant member of the approved families; see [[fixed-input-outcome-equivalence]] for that second step and for the $Y$ self-fields where it fails.
