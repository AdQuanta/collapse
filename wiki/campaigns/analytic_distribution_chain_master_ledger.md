# Analytic \(P(\theta)\) / \(R(\theta)\) Master Ledger — endpoint chain

This is the **single atomic progress ledger for the endpoint chain geometry** in the analytic-distribution program.

Use the repository's endpoint-chain coupling convention, with the central qubit coupled to detector site 1, in every row.
...
A row may be marked `PROVED` only after the requested analytical program has been addressed for the **endpoint chain** geometry, including:
