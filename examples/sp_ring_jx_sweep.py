"""
Single-Pixel Ring – Jx Sweep
==============================

Sweep Jx from 0.01 to 10 (20 log-spaced values) with fixed
Jz=0, J=1, hx=0, hz=0.5, ring connectivity.

Usage::

    python examples/sp_ring_jx_sweep.py
    python examples/sp_ring_jx_sweep.py -N 7 --workers 4
    python examples/sp_ring_jx_sweep.py --save figures/jx_sweep --bloch
"""
...
def build_scenarios(N: int) -> list[dict]:
    ...
    for Jx in JX_VALUES:
        ...
        title_line = f"SP[ring] | J=1  Jx={Jx:g}  Jz=0  hx=0  hz=0.5"
        scenarios.append(
            {
...
def main():
    parser = argparse.ArgumentParser(
        description="SP ring – Jx sweep (Jz=0, J=1, hx=0, hz=0.5)"
    )
    ...
    log_file = log_dir / f"sp_ring_jx_sweep_{datetime.now():%Y%m%d_%H%M%S}.log"
    ...
    logger.info(
        f"=== SP Ring Jx Sweep: {n} scenarios, N={args.N} (D={2**args.N}), "
