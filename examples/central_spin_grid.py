"""
Central-Spin – Full 3-Parameter Grid Scan
==========================================

Sweep **all** combinations of (Jx, Jz, hz) over
{0, 0.1, 0.2, 0.5, 1, 2, 5, 10} with hx=0.

8³ = 512 scenarios, each evaluated at 9 time points.

Usage::

    python examples/central_spin_grid.py
    python examples/central_spin_grid.py -N 7 --workers 4
    python examples/central_spin_grid.py --save figures/cs_grid
    python examples/central_spin_grid.py --save figures/cs_grid --bloch
"""
...
from collapse.hamiltonians.quspin_hamiltonians import CentralSpinHamiltonianQuSpin
...
def build_scenarios(N: int) -> list[dict]:
    ...
    ----------
    N : int
        Total number of qubits (1 central + N-1 bath).
    ...
    for Jx in GRID_VALUES:
        for Jz in GRID_VALUES:
            for hz in GRID_VALUES:
                ...
                title_line = f"CS | Jx={Jx:g}  Jz={Jz:g}  hx=0  hz={hz:g}"
                scenarios.append(
                    {
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Central-spin grid scan – all (Jx, Jz, hz) combos, " "hx=0, no disorder"
        )
    )
    parser.add_argument(
        "-N",
        ...
        default=7,
        dest="N",
        help="Total number of qubits (1 central + N-1 bath, default: 7).",
    )
    parser.add_argument(
        "--no-theta",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Save figures to directory (e.g. figures/cs_grid)",
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    log_file = log_dir / f"central_spin_grid_{datetime.now():%Y%m%d_%H%M%S}.log"
    ...
    logger.info(
        f"=== CS Grid Scan: {n} scenarios, N={args.N} (D={2**args.N}), "
