"""
Single-Pixel All-to-All – Full 3-Parameter Grid Scan (Scaled J)
================================================================

Same grid as ``single_pixel_ring_grid.py`` but with **all-to-all**
connectivity and the intra-pixel coupling ``J`` scaled by the number
of pixel spins:  J_eff = J_bare / N_pixel.

This keeps the total interaction energy per spin O(1) as N grows.

Usage::

    python examples/sp_ata_grid.py
    python examples/sp_ata_grid.py -N 7 --workers 4
    python examples/sp_ata_grid.py --save figures/sp_ata --bloch
"""
...
def build_scenarios(N: int) -> list[dict]:
    ...
    with J = 1 / N_pixel (scaled coupling).
    """
    N_pixel = N - 1
    J_scaled = 1.0 / N_pixel
    scenarios: list[dict] = []
    ...
    for Jx in GRID_VALUES:
        for Jz in GRID_VALUES:
            for hz in GRID_VALUES:
                ...
                title_line = (
                    f"SP[ata] | J=1/{N_pixel}={J_scaled:.4f}  "
                    f"Jx={Jx:g}  Jz={Jz:g}  hx=0  hz={hz:g}"
                )
                scenarios.append(
                    {
...
def main():
    parser = argparse.ArgumentParser(
        description="SP all-to-all grid scan – J scaled by N_pixel"
    )
    ...
    log_file = log_dir / f"sp_ata_grid_{datetime.now():%Y%m%d_%H%M%S}.log"
    ...
    logger.info(
        f"=== SP ATA Grid Scan: {n} scenarios, N={args.N} (D={2**args.N}), "
