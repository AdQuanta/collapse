"""
Single-Pixel Ring – Jx Sweep × Jz × hz Placement
===================================================

Sweep ``Jx`` from 0.01 to 10 (20 log-spaced values) for several fixed
``Jz`` values and all three ``hz``-placement modes, with J=1, hx=0,
ring connectivity.

Jz values tested: 0, 0.1, 1.0, 10.0
...
Total scenarios = 20 × 4 × 3 = 240.

In addition to per-scenario histograms, summary figures show the
spherical-harmonic projections (density and ratio) vs ``Jx``, organised
as rows = time and columns = (Jz value × hz mode).

Usage::

    python examples/sp_ring_jx_jz_sweep_hz_placement.py
    python examples/sp_ring_jx_jz_sweep_hz_placement.py -N 7 --workers 4
    python examples/sp_ring_jx_jz_sweep_hz_placement.py --save figures/jx_jz_hz --bloch
"""
...
# 20 log-spaced Jx values from 0.01 to 10
JX_VALUES = np.round(np.logspace(np.log10(0.01), np.log10(10), 20), 4).tolist()

# Fixed Jz values to test
JZ_VALUES = [0.0, 0.1, 1.0, 10.0]
...
def build_scenarios(N: int) -> list[dict]:
    """Enumerate Jx × Jz × hz-placement scenarios."""
    scenarios: list[dict] = []
    for Jx in JX_VALUES:
        for Jz in JZ_VALUES:
            for hz_label, hz_val, hz0_val in HZ_MODES:
                ...
                combo_label = f"Jz={Jz:g}+{hz_label}"
                title_line = (
...
def plot_sh_summary(results, save_dir, logger):
    """Produce two summary figures (density + ratio): SH projections vs Jx
    for each (Jz × hz mode) combo.  Rows = time, columns = combo.
    """
    projector = SphericalHarmonicProjector(
        l_max=L_MAX, n_theta=max(L_MAX + 1, 16), n_phi=max(2 * L_MAX + 1, 32)
    )

    # Build ordered combo labels: Jz=0+hz0_only, Jz=0+hz_only, ...
    combo_labels = []
    for Jz in JZ_VALUES:
        for hz_label, _, _ in HZ_MODES:
            combo_labels.append(f"Jz={Jz:g}+{hz_label}")
    ...
    for kind_idx, (kind_name, file_suffix, suptitle) in enumerate(
        [
            ("density", "density", r"SH projections of $\rho_0$ density vs $J_x$"),
            ("ratio", "ratio", r"SH projections of $h_0/(h_0{+}h_1)$ ratio vs $J_x$"),
        ]
    ):
        fig, axes = plt.subplots(
            n_times,
            n_combos,
            figsize=(3.5 * n_combos, 2.5 * n_times),
        ...
        for col, cl in enumerate(combo_labels):
            for row, t_val in enumerate(TIMES):
                ...
                if row == n_times - 1:
                    ax.set_xlabel(r"$J_x$", fontsize=7)
        ...
        if save_dir:
            out = save_dir / f"sh_summary_jx_jz_hz_{file_suffix}.png"
...
def main():
    parser = argparse.ArgumentParser(
        description="SP ring – Jx sweep × Jz × hz placement"
    )
    ...
    log_file = (
        log_dir
        / f"sp_ring_jx_jz_sweep_hz_placement_{datetime.now():%Y%m%d_%H%M%S}.log"
    )
    ...
    logger.info(
        f"=== SP Ring Jx Sweep × Jz × hz Placement: {n} scenarios, N={args.N} "
