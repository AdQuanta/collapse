"""
Single-Pixel Chain (+−) – Full 3-Parameter Grid Scan
=====================================================

Sweep **all** combinations of (Jx, Jz, hz) over
{0, 0.1, 0.2, 0.5, 1, 2, 5, 10} with hx=0, J=0, Jpm=1,
open-chain connectivity and +− (XY) intra-pixel interaction.

8³ = 512 scenarios, each evaluated at 9 time points.

Usage::

    python examples/single_pixel_chain_pm_grid.py
    python examples/single_pixel_chain_pm_grid.py -N 7 --workers 4
    python examples/single_pixel_chain_pm_grid.py --save figures/sp_chain_pm
    python examples/single_pixel_chain_pm_grid.py --save figures/sp_chain_pm --bloch
"""
    ...
    for Jx in GRID_VALUES:
        for Jz in GRID_VALUES:
            for hz in GRID_VALUES:
                ...
                title_line = (
                    f"SP[chain,+-] | J=0  Jpm=1  Jx={Jx:g}  Jz={Jz:g}  "
                    f"hx=0  hz={hz:g}"
                )
                scenarios.append(
                    {
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Single-Pixel chain +-grid scan – all (Jx, Jz, hz) combos, "
            "hx=0, J=0, Jpm=1, no disorder"
        )
    )
    parser.add_argument(
        "-N",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Save figures to directory (e.g. figures/sp_chain_pm)",
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    log_file = (
        log_dir / f"single_pixel_chain_pm_grid_{datetime.now():%Y%m%d_%H%M%S}.log"
    )
    ...
    logger.info(
        f"=== SP Chain +- Grid Scan: {n} scenarios, N={args.N} (D={2**args.N}), "
