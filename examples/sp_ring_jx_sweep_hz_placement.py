"""
Single-Pixel Ring – Jx Sweep × hz Placement
=============================================

Sweep Jx from 0.01 to 10 (20 log-spaced values) with fixed
Jz=0, J=1, hx=0, ring connectivity.

Three ``hz`` placement modes are tested for each ``Jx`` value:

* **central only** – ``hz0=0.5, hz=0``
* **detector only** – ``hz0=0, hz=0.5``
* **all qubits**  – ``hz0=None, hz=0.5`` (uniform, identical to
  the original sweep)
...
of ``Jx`` for each hz-placement mode.

Usage::

    python examples/sp_ring_jx_sweep_hz_placement.py
    python examples/sp_ring_jx_sweep_hz_placement.py -N 7 --workers 4
    python examples/sp_ring_jx_sweep_hz_placement.py --save figures/jx_hz --bloch
"""
...
# 20 log-spaced values from 0.01 to 10
...
def build_scenarios(N: int) -> list[dict]:
    """Enumerate Jx × hz-placement scenarios."""
    scenarios: list[dict] = []
    for Jx in JX_VALUES:
...
def plot_sh_summary(results, save_dir, logger):
    ...
    for kind_idx, (kind_name, file_suffix, suptitle) in enumerate([
        ...
        if save_dir:
            out = save_dir / f"sh_summary_jx_hz_{file_suffix}.png"
...
def main():
    parser = argparse.ArgumentParser(
        description="SP ring – Jx sweep × hz placement"
    )
    ...
    log_file = log_dir / f"sp_ring_jx_sweep_hz_placement_{datetime.now():%Y%m%d_%H%M%S}.log"
    ...
    logger.info(
        f"=== SP Ring Jx Sweep × hz Placement: {n} scenarios, N={args.N} "
