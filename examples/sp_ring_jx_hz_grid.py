"""
Single-Pixel Ring – Jx × hz Grid  (SH Heatmaps)
==================================================

2-D sweep over ``Jx`` (30 log-spaced, 0.01–10) and ``hz`` (30 linearly
spaced, 0.01–2) with fixed J=1, Jz=0, hx=0, ring connectivity.
...
results are visualised as heatmaps on the (Jx, hz) plane:
...
Usage::
    ...
    python examples/sp_ring_jx_hz_grid.py
    python examples/sp_ring_jx_hz_grid.py -N 7 --workers 4
    python examples/sp_ring_jx_hz_grid.py --save figures/jx_hz_grid
"""
...
# 30 log-spaced Jx values from 0.01 to 10
JX_VALUES = np.round(np.logspace(np.log10(0.01), np.log10(10), 30), 4).tolist()

# 30 linearly-spaced hz values from 0.01 to 2
HZ_VALUES = np.round(np.linspace(0.01, 2.0, 30), 4).tolist()

# hz placement modes: (label, hz_factory, hz0_factory)
# Each factory takes the swept hz_value and returns the param to pass.
HZ_MODES = [
    ...
    for Jx in JX_VALUES:
        for hz_sweep in HZ_VALUES:
            for hz_label, hz_fn, hz0_fn in HZ_MODES:
                ...
                title_line = (
                    f"SP[ring] | J=1  Jx={Jx:g}  Jz=0  hx=0  "
...
def _run_scenario(scenario: dict) -> dict:
    ...
    try:
        ...
        return {
            ...
            "hz_sweep": scenario.get("hz_sweep", 0),
...
def plot_heatmaps(results, save_dir, logger):
    ...
    # {hz_label: {(Jx, hz_sweep): {t: (density_decomp, ratio_decomp)}}}
    ...
    for ml in mode_labels:
        for t_val in TIMES:
            ...
            for kind_idx, kind_name in enumerate(kind_names):
                # Build 2-D arrays: shape (n_hz, n_jx)
                grids = [np.full((n_hz, n_jx), np.nan) for _ in range(4)]
                ...
                for c in range(4):
                    ax = axes[kind_idx, c]
                    im = ax.pcolormesh(
                        jx_arr, hz_arr, grids[c],
                        cmap="viridis", shading="nearest",
                    )
                    ax.set_xscale("log")
                    ax.set_xlabel(r"$J_x$", fontsize=9)
                    ax.set_ylabel(r"$h_z$", fontsize=9)
                    ax.set_title(
            ...
            fig.suptitle(
                rf"SH projections on $J_x$–$h_z$ plane  |  {ml}  |  $t={t_val:g}$",
                fontsize=13, fontweight="bold",
            )

            if save_dir:
                out = save_dir / f"sh_heatmap_{ml}_t{t_val:g}.png"
...
def main():
    parser = argparse.ArgumentParser(
        description="SP ring – Jx × hz grid with SH heatmaps"
    )
    ...
    log_file = log_dir / f"sp_ring_jx_hz_grid_{datetime.now():%Y%m%d_%H%M%S}.log"
    ...
    logger.info(
        f"=== SP Ring Jx × hz Grid: {n} scenarios ({len(JX_VALUES)}×{len(HZ_VALUES)}), "
