...
"""Run one 100-case batch of the fixed-hz0=0.1, N=15 Sobol campaign."""
...
HZ0 = 0.1
DETECTOR_N = 15
TOTAL_CONFIGURATIONS = 400
BATCH_SIZE = 100
FAMILY_SEEDS = {"jy_zero": 20260731, "jy_nonzero": 20260801}


def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description=__doc__)
    result.add_argument("--family", choices=tuple(FAMILY_SEEDS), required=True)
...
def _copy_provenance_once(campaign: SobolCampaign) -> None:
    ...
    try:
        ...
        for relative in (
            "examples/run_zeus_sobol_hz0_0p1_N15.py",
            "hpc/zeus_sobol_hz0_0p1_N15_array.pbs",
            "hpc/submit_zeus_sobol_hz0_0p1_N15.sh",
            "hpc/zeus_sobol_hz0_0p1_N15.md",
        ):
...
def main() -> None:
    ...
    if settings.hz0 != HZ0 or settings.count != TOTAL_CONFIGURATIONS or settings.sizes != (DETECTOR_N,):
        raise RuntimeError("production invariants changed: require hz0=0.1, count=400, N=15")
    ...
    if args.dry_run:
        ...
        print(f"[{timestamp()}] dry_run family={args.family} batch={args.batch_index} ids=config_{start:03d}..config_{stop-1:03d}")
        return
    ...
    print(
        f"[{timestamp()}] family={args.family} N={DETECTOR_N} hz0={HZ0} "
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                _atomic_json(
                    ...
                    {"status": "failed", "simulation": args.family, "configuration": point,
            ...
            rows.append(result)
            print(
                f"[{timestamp()}] family={args.family} batch={args.batch_index} "
    ...
    print(f"[{timestamp()}] family={args.family} batch={args.batch_index} checkpoint={validated}/{BATCH_SIZE}", flush=True)
