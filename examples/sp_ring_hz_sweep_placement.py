"""
Single-Pixel Ring – hz Sweep × hz Placement
=============================================

Sweep hz from 0.01 to 2 (20 linearly-spaced values) with fixed
Jx=0.1, Jz=0, J=1, hx=0, ring connectivity.

Three ``hz`` placement modes are tested for each ``hz`` value:

* **central only** – ``hz0=<value>, hz=0``
* **detector only** – ``hz0=0, hz=<value>``
* **all qubits**  – ``hz0=None, hz=<value>``

In addition to per-scenario histograms, summary figures show the
spherical-harmonic projections of density and ratio as a function
of ``hz`` for each hz-placement mode.

Usage::

    python examples/sp_ring_hz_sweep_placement.py
    python examples/sp_ring_hz_sweep_placement.py -N 7 --workers 4
    python examples/sp_ring_hz_sweep_placement.py --save figures/hz_sweep --bloch
"""
...
# 20 linearly-spaced hz values from 0.01 to 10
HZ_VALUES = np.round(np.linspace(0.01, 10.0, 20), 4).tolist()

# hz placement modes: (label, hz_factory, hz0_factory)
HZ_MODES = [
    ("hz0_only", lambda v: 0.0, lambda v: v),  # central qubit only
    ("hz_only", lambda v: v, lambda v: 0.0),  # detector qubits only
    ("hz_all", lambda v: v, lambda v: None),  # uniform on all qubits
]
...
def build_scenarios(N: int) -> list[dict]:
    """Enumerate hz × hz-placement scenarios."""
    scenarios: list[dict] = []
    for hz_sweep in HZ_VALUES:
        for hz_label, hz_fn, hz0_fn in HZ_MODES:
            hz_param = hz_fn(hz_sweep)
            hz0_param = hz0_fn(hz_sweep)
            params = dict(
                N_pixel=N - 1,
                J=1.0,
                Jx=0.1,
            ...
            hz0_str = f"{hz0_param:g}" if hz0_param is not None else "—"
            title_line = (
                f"SP[ring] | J=1  Jx=0.1  Jz=0  hx=0  "
                f"hz={hz_param:g}  hz0={hz0_str}  ({hz_label})"
            )
            scenarios.append(
                {
...
def plot_sh_summary(results, save_dir, logger):
    """Produce two summary figures (density + ratio): SH projections vs hz
    for each hz mode.  One row per time value, one column per hz-placement.
    """
    ...
    # {hz_label: {t_val: [(sweep_val, (density_decomp, ratio_decomp)), ...]}}
    data = {ml: {t: [] for t in TIMES} for ml in mode_labels}

    for res in results:
        if "error" in res:
            continue
        hz_label = res["hz_label"]
        sweep_val = res["sweep_val"]
        for td in res["time_results"]:
            decomps = compute_sh_projections(td, projector)
            data[hz_label][td["t"]].append((sweep_val, decomps))

    for ml in mode_labels:
        for t in TIMES:
            data[ml][t].sort(key=lambda x: x[0])
    ...
    for kind_idx, (kind_name, file_suffix, suptitle) in enumerate(
        [
            ("density", "density", r"SH projections of $\rho_0$ density vs $h_z$"),
            ("ratio", "ratio", r"SH projections of $h_0/(h_0{+}h_1)$ ratio vs $h_z$"),
        ]
    ):
        fig, axes = plt.subplots(
            n_times,
            n_modes,
            figsize=(5 * n_modes, 2.5 * n_times),
        ...
        for col, ml in enumerate(mode_labels):
            for row, t_val in enumerate(TIMES):
                ax = axes[row, col]
                entries = data[ml][t_val]
                if not entries:
                    continue
                ...
                ax.plot(
                    ...
                    ms=3,
                    lw=1.2,
                    label=r"$\sum_{\ell\geq2}|a_{\ell m}|^2$",
                )
                ...
                if row == n_times - 1:
                    ax.set_xlabel(r"$h_z$", fontsize=8)
                if row == 0 and col == n_modes - 1:
                    ax.legend(fontsize=6, loc="upper right")

        fig.suptitle(suptitle, fontsize=12, fontweight="bold")

        if save_dir:
            out = save_dir / f"sh_summary_hz_{file_suffix}.png"
...
# ===================================================================
def main():
    parser = argparse.ArgumentParser(description="SP ring – hz sweep × hz placement")
    ...
    log_file = (
        log_dir / f"sp_ring_hz_sweep_placement_{datetime.now():%Y%m%d_%H%M%S}.log"
    )
    ...
    logger.info(
        f"=== SP Ring hz Sweep × hz Placement: {n} scenarios, N={args.N} "
    ...
    for res in results:
        ...
        fig.suptitle(
            f"#{idx+1:03d}  {title_line}", fontsize=10, fontweight="bold", y=1.02
        )
