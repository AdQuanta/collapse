"""
Single-Pixel Ring – Single-parameter Born diagnostic with joint (θ,φ)
======================================================================

Simulates a **single** parameter choice:
...
3. **Joint (θ, φ) heatmaps** for subsystems 0 and 1 side by side,
   showing the 2-D density on the Bloch sphere.
4. **Bloch-sphere** scatter (3-D) for each time.
...
Usage::
    ...
    python examples/sp_ring_single_joint.py -N 7 9 11 --save figures/single_joint
    python examples/sp_ring_single_joint.py -N 9 --save figures/single_joint --Jpm 0.5
"""
...
def _plot_joint_theta_phi(ax, theta, phi_az, title, n_theta=30, n_phi=30):
    r"""Draw joint (θ, φ) 2-D density heatmap."""
    theta_edges = np.linspace(0, np.pi, n_theta + 1)
    phi_edges = np.linspace(0, 2 * np.pi, n_phi + 1)

    hist, _, _ = np.histogram2d(theta, phi_az, bins=[theta_edges, phi_edges])

    # Normalise each θ-row by solid-angle element sin(θ) Δθ Δφ to get
    # a density that is comparable across latitude bands.
    theta_centers = (theta_edges[:-1] + theta_edges[1:]) / 2
    dtheta = theta_edges[1] - theta_edges[0]
    dphi = phi_edges[1] - phi_edges[0]
    solid_angle = np.sin(theta_centers) * dtheta * dphi  # per bin
    with np.errstate(divide="ignore", invalid="ignore"):
        density = hist / solid_angle[:, None]
    ...
    ax.set_xlim(0, 2 * np.pi)
    ax.set_ylim(0, np.pi)
    ax.invert_yaxis()  # θ=0 (north pole) at top
    ax.tick_params(labelsize=5)
    ax.set_title(title, fontsize=7, pad=3)
    cb = plt.colorbar(im, ax=ax, pad=0.02, fraction=0.046)
    cb.ax.tick_params(labelsize=4)
    cb.set_label("density / sr", fontsize=5)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "SP ring – single parameter set, Born diagnostic with joint "
            "(θ, φ) distribution.  Default: J=1, Jx=0.01/√N_pixel, "
            "Jz=0, hz=hz0=0.1."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--Jpm", type=float, default=0.0,
        help="Intra-pixel σ⁺σ⁻ + σ⁻σ⁺ (XY) coupling strength (default: 0.0)",
    )
    parser.add_argument(
        "--hz", type=float, default=HZ,
        help=f"Longitudinal field hz=hz0 (default: {HZ})",
    )
    ...
    log_file = log_dir / f"sp_ring_single_joint_{datetime.now():%Y%m%d_%H%M%S}.log"
    ...
    jpm_str = f", Jpm={args.Jpm:g}" if args.Jpm != 0.0 else ""
    logger.info(
        f"=== SP Ring single-param Born+joint diagnostic: "
        f"N={args.N_list}, J={args.J:g}, Jx={JX_UNSCALED}{jpm_str}, hz={args.hz:g}, "
    ...
    for N in args.N_list:
        ...
        # ---- Figure: 5 rows × ntimes cols ----
        # Row 1: θ-histogram
        # Row 2: φ-histogram
        # Row 3: joint (θ, φ) – subsystem 0
        # Row 4: joint (θ, φ) – subsystem 1
        # Row 5: Bloch sphere
        n_rows = 5
        fig = plt.figure(
            figsize=(4.0 * ntimes, 3.2 * n_rows),
            dpi=120,
        ...
        for col, td in enumerate(time_data):
            # Row 1: θ-histogram
            ax_th = fig.add_subplot(n_rows, ntimes, col + 1)
            _plot_theta_hist(
                ax_th, td["z0"], td["z1"],
            ...
            # Row 2: φ-histogram
            ax_ph = fig.add_subplot(n_rows, ntimes, ntimes + col + 1)
            _plot_phi_hist(
            ...
            # Row 3: joint (θ, φ) – subsystem 0
            ax_j0 = fig.add_subplot(n_rows, ntimes, 2 * ntimes + col + 1)
            _plot_joint_theta_phi(
                ax_j0, td["theta0"], td["phi_az0"],
                title=f"sub-0  t={td['t']:g}",
            )

            # Row 4: joint (θ, φ) – subsystem 1
            ax_j1 = fig.add_subplot(n_rows, ntimes, 3 * ntimes + col + 1)
            _plot_joint_theta_phi(
                ax_j1, td["theta1"], td["phi_az1"],
                title=f"sub-1  t={td['t']:g}",
            )

            # Row 5: Bloch sphere
            ax_bl = fig.add_subplot(
                n_rows, ntimes, 4 * ntimes + col + 1, projection="3d"
            )
            _plot_bloch(
                ax_bl,
                td["bx0"], td["by0"], td["z0"],
        ...
        jpm_title = f"  Jpm={args.Jpm:g}" if args.Jpm != 0.0 else ""
        fig.suptitle(
            f"SP[ring]  N={N}  |  J={args.J:g}  Jx={JX_UNSCALED}/√{N_pixel}"
            f"{jpm_title}  Jz=0  hz=hz0={args.hz:g}",
            fontsize=11,
            fontweight="bold",
            y=1.02,
        )
