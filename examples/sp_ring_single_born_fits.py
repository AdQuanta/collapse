"""
Single-Pixel Ring – Born diagnostic + wrapped-distribution fits
================================================================
...
Fixed parameters:
    ...
    J = 1,  Jx = 0.01 / sqrt(N_pixel),  Jz = 0,  hz = 0.1,  hx = 0

Times: t = 100, 1 000, 10 000, 1 000 000.  Uses symmetry sectors.
...
Usage::
    ...
    python examples/sp_ring_single_born_fits.py -N 7 9 --save figures/fits
"""
...
def wrapped_gaussian_pdf(theta, mu, sigma, n_terms=20):
    ...
    for p in range(1, n_terms + 1):
        result += (1.0 / np.pi) * np.exp(-0.5 * p**2 * sigma**2) * np.cos(
            p * (theta - mu)
        )
...
def _plot_density_with_fits(ax, centers, density, gauss_popt, cauchy_popt,
                            title, log_scale=False):
    ...
    if gauss_popt is not None:
        ...
        ax.plot(theta_fine, y_gauss, "-", color="darkorange", lw=1.5,
                label=rf"W-Gauss $\sigma$={sigma:.2f}")

    if cauchy_popt is not None:
        A, rho = cauchy_popt
        y_cauchy = A * wrapped_cauchy_pdf(theta_fine, 0.0, rho)
        ax.plot(theta_fine, y_cauchy, "-", color="green", lw=1.5,
                label=rf"W-Cauchy $\rho$={rho:.3f}")

    if log_scale:
        ax.set_yscale("log")
        ax.set_ylim(bottom=max(density[density > 0].min() * 0.3, 1e-6) if np.any(density > 0) else 1e-6)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "SP ring -- single param set, Born diagnostic + wrapped fits.  "
    ...
    parser.add_argument(
        "--hz", type=float, default=0.1,
        help="Field value on the ring spins (default: 0.1)",
    )
    ...
    log_file = (
        log_dir / f"sp_ring_single_born_fits_{datetime.now():%Y%m%d_%H%M%S}.log"
    )
    ...
    logger.info(
        f"=== SP Ring Born + fits: N={args.N_list}, "
        f"Jx={JX_UNSCALED}, hz={HZ}, {len(TIMES)} times ==="
    )
    ...
    for N in args.N_list:
        ...
        for scen in SCENARIOS:
            ...
            logger.info(
                f"    {len(sectors)} sectors built ({time.time()-t0_wall:.1f}s)"
            )
            ...
            for t_val in TIMES:
                ...
                centers, density, gauss_popt, cauchy_popt = (
                    fit_wrapped_distributions(theta0)
                )
                ...
                time_data.append(
                    dict(
                        ...
                        S=S,
                        fit_centers=centers, fit_density=density,
                        gauss_popt=gauss_popt, cauchy_popt=cauchy_popt,
                    )
                )
            ...
            fig = plt.figure(
                figsize=(4.0 * ntimes, 10.0), dpi=120, constrained_layout=True,
            )
            for col, td in enumerate(time_data):
                ...
                ax_bl = fig.add_subplot(
                    3, ntimes, 2 * ntimes + col + 1, projection="3d",
                )
                _plot_bloch(
                    ax_bl,
                    td["bx0"], td["by0"], td["z0"],
            ...
            fig.suptitle(
                f"SP[ring] N={N} | J=1 Jx={JX_UNSCALED}/sqrt({N_pixel}) "
                f"Jz=0 hz={HZ} {label}",
        ...
        fig2.suptitle(
            f"Wrapped-distribution fits  |  N={N}  "
            f"Jx={JX_UNSCALED}/sqrt({N_pixel})",
