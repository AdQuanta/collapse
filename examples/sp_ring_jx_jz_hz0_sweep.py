"""
Single-Pixel Ring – Jx × Jz × hz0 Sweep  (no detector field, hz=0)
====================================================================

3-D parameter sweep over ``Jx``, ``Jz``, and ``hz0`` with **hz = 0**
(no field on the detector qubits), hx = 0, J = 1, ring connectivity.

The intra-pixel couplings ``Jx`` and ``Jz`` are scaled by
``1 / N_pixel`` to keep the energy per spin O(1).  The central-qubit
field ``hz0`` acts on a single spin and is **not** scaled.

Sweep ranges
~~~~~~~~~~~~
* ``Jx``: 11 log-spaced values from 0.1 to 10
* ``Jz``: 11 log-spaced values from 0.1 to 10
* ``hz0``: 11 log-spaced values from 0.1 to 10

Total scenarios = 11 × 11 × 11 = 1331.

Three types of plots are produced (all saved progressively):

1. **Histograms + Bloch** (per scenario):
   ``histograms/hz0={hz0}/Jz={Jz}/Jx={Jx}.png``

2. **SH curves vs Jx** (per hz0, Jz):
   ``sh_curves/hz0={hz0}/Jz={Jz}_{density|ratio}.png``

3. **SH heatmaps on Jx–Jz plane** (per hz0, time):
   ``sh_heatmaps/hz0={hz0}/t={t}_{density|ratio}.png``

Usage::

    python examples/sp_ring_jx_jz_hz0_sweep.py -N 7 --workers 4 --save figures/sweep3d_hz0
    python examples/sp_ring_jx_jz_hz0_sweep.py -N 7 --workers 4 --save figures/sweep3d_hz0 --hz0-idx 3
"""
...
HZ0_VALUES = np.round(np.logspace(np.log10(0.1), np.log10(10), 11), 4).tolist()
...
# Build all scenarios
# ===================================================================
def build_scenarios(N: int, hz0_idx: int | None = None) -> list[dict]:
    """Enumerate Jx × Jz × hz0 scenarios.

    Jx and Jz are scaled by 1/N_pixel.  hz0 is a single-site field
    and is **not** scaled.  If *hz0_idx* is given, only that hz0 value
    is used.
    """
    N_pixel = N - 1
    scenarios: list[dict] = []
    hz0_list = [HZ0_VALUES[hz0_idx]] if hz0_idx is not None else HZ0_VALUES
...
def save_histograms(results, save_dir, logger):
    ...
    for res in results:
        if "error" in res:
            continue
        hz0, Jz, Jx = res["hz0"], res["Jz"], res["Jx"]
        tr = res["time_results"]

        out_dir = save_dir / "histograms" / f"hz0={hz0:g}" / f"Jz={Jz:g}"
...
def save_sh_curves(results, save_dir, logger):
    ...
    for (hz0, Jz), time_data in grouped.items():
        out_dir = save_dir / "sh_curves" / f"hz0={hz0:g}"
        out_dir.mkdir(parents=True, exist_ok=True)
        ...
        for kind_idx, (kind_name, suptitle_base) in enumerate(kind_info):
            ...
            fig.suptitle(
                f"{suptitle_base}  |  hz0={hz0:g}  Jz={Jz:g}",
                fontsize=11,
                fontweight="bold",
            )
...
# ===================================================================
# Plot type 3: SH heatmaps on Jx-Jz plane (per hz0, time)
# ===================================================================
def save_sh_heatmaps(results, save_dir, logger):
    """Save SH heatmaps on the Jx-Jz plane for each (hz0, time) pair."""
    projector = SphericalHarmonicProjector(
        l_max=L_MAX, n_theta=max(L_MAX + 1, 16), n_phi=max(2 * L_MAX + 1, 32)
    )
    ...
    # Group: {hz0: {(Jx, Jz): {t: (density, ratio)}}}
    grouped: dict[float, dict[tuple[float, float], dict[float, tuple]]] = {}
    for res in results:
        if "error" in res:
            continue
        hz0 = res["hz0"]
        if hz0 not in grouped:
            grouped[hz0] = {}
        key = (res["Jx"], res["Jz"])
        grouped[hz0][key] = {}
        for td in res["time_results"]:
            grouped[hz0][key][td["t"]] = compute_sh_projections(td, projector)
    ...
    for hz0, grid_data in grouped.items():
        out_dir = save_dir / "sh_heatmaps" / f"hz0={hz0:g}"
        out_dir.mkdir(parents=True, exist_ok=True)
        ...
        for t_val in TIMES:
            for kind_idx, (kind_name, suptitle_base) in enumerate(kind_info):
                ...
                fig.suptitle(
                    f"{suptitle_base}  |  hz0={hz0:g}  t={t_val:g}",
                    fontsize=12,
                    fontweight="bold",
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="SP ring – Jx × Jz × hz0 sweep (hz=0, Jx/Jz scaled by 1/N_pixel)"
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            f"If given, run only the hz0 value at this index "
            f"(0..{len(HZ0_VALUES)-1}).  Useful for splitting work across jobs."
        ),
    )
    args = parser.parse_args()

    if args.hz0_idx is not None and not (0 <= args.hz0_idx < len(HZ0_VALUES)):
        parser.error(f"--hz0-idx must be in 0..{len(HZ0_VALUES)-1}")

    scenarios = build_scenarios(args.N, hz0_idx=args.hz0_idx)
    ...
    log_file = log_dir / f"sp_ring_jx_jz_hz0_sweep_{datetime.now():%Y%m%d_%H%M%S}.log"
    ...
    logger.info(
        f"=== SP Ring Jx × Jz × hz0 Sweep (hz=0): {n} scenarios "
        f"({len(JX_VALUES)}×{len(JZ_VALUES)}×{len(HZ0_VALUES)}), "
        f"N={args.N} (N_pixel={N_pixel}, D={2**args.N}), "
        f"J=1, Jx/Jz scaled by 1/{N_pixel}, hz0 unscaled, "
