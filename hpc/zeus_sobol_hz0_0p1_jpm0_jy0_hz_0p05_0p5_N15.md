# Zeus Sobol scan: hz in [0.05,0.5], hz0=0.1, Jpm=0, Jy=0, N=15

From the repository root on a Zeus login node, run:

```bash
bash hpc/submit_zeus_sobol_hz0_0p1_jpm0_jy0_hz_0p05_0p5_N15.sh
```
...
- `Jpm=0` exactly;
- `Jy=0` exactly;
- `hz` is sampled logarithmically in `[0.05,0.5]`;
- `N=15` and `t=1e6`.

Only `J`, `hz`, and `Jx` are sampled. `J` uses the logarithmic interval
`[1e-3,10]`, while `hz` uses `[0.05,0.5]`. `Jx` is sampled conditionally so that
...
```bash
export RUN_ROOT="$PWD/work/my_jpm0_jy0_hz_band_scan"
bash hpc/submit_zeus_sobol_hz0_0p1_jpm0_jy0_hz_0p05_0p5_N15.sh
```
