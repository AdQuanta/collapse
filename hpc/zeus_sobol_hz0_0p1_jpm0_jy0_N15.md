# Zeus Sobol scan: hz0=0.1, Jpm=0, Jy=0, N=15

From the repository root on a Zeus login node, run:

```bash
bash hpc/submit_zeus_sobol_hz0_0p1_jpm0_jy0_N15.sh
```
...
Only `J`, `hz`, and `Jx` are sampled. Candidate `J` and `hz` coordinates use
the logarithmic interval `[1e-3,10]`; `Jx` is sampled conditionally so that
...
```bash
export RUN_ROOT="$PWD/work/my_jpm0_jy0_scan"
bash hpc/submit_zeus_sobol_hz0_0p1_jpm0_jy0_N15.sh
```
