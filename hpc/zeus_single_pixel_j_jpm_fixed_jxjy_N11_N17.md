# Zeus Handoff: Single-Pixel J-Jpm Scan With Fixed Weak XX/YY Coupling

Date: 2026-07-03

## Purpose

Run the single-pixel ring Hamiltonian with weak central-qubit detector coupling

$$
J_x=J_y=10^{-2}
$$

and scan the detector \(J\)-\(J_{\pm}\) plane for \(N=11,\ldots,17\). The run uses QuSpin and enforces the existing cyclic detector-site shift sector path through clean `ring` connectivity and uniform `all` central coupling.
...
+
J_y^{\rm phys}Y_0\sum_iY_i.
$$

The PBS defaults set

$$
J_x^{\rm unscaled}=J_y^{\rm unscaled}=10^{-2}.
$$
...
\qquad
J_y^{\rm phys}=J_y^{\rm unscaled}/\sqrt{N}.
$$

Thus this is safely perturbative. To force the physical coupling itself to be \(10^{-2}\), submit with \(JX_UNSCALED\) and \(JY_UNSCALED\) increased by \(\sqrt{N}\), but the default here intentionally follows the existing scan convention.

## Default Grid

The PBS script defaults to an 11x11 detector grid:
...
```bash
hpc/zeus_single_pixel_j_jpm_fixed_jxjy_N11_N17.pbs
```
...
RUN_ROOT="figures/zeus_single_pixel_j_jpm_fixed_jxjy_N11_N17_pilot_${RUN_TAG}"
LOG_ROOT="logs/zeus_single_pixel_j_jpm_fixed_jxjy_N11_N17_pilot_${RUN_TAG}"
...
This tests \(N=11,h_{z0}=0\). Inspect:
...
cd "$HOME/research/collapse"
RUN_TAG="$(date +%Y%m%d_%H%M%S)"
RUN_ROOT="figures/zeus_single_pixel_j_jpm_fixed_jxjy_N11_N17_${RUN_TAG}"
LOG_ROOT="logs/zeus_single_pixel_j_jpm_fixed_jxjy_N11_N17_${RUN_TAG}"
mkdir -p "$RUN_ROOT" "$LOG_ROOT"
...
cd "$HOME/research/collapse"
RUN_TAG="$(date +%Y%m%d_%H%M%S)"
RUN_ROOT="figures/zeus_single_pixel_j_jpm_fixed_jxjy_N11_N17_tiny_pilot_${RUN_TAG}"
LOG_ROOT="logs/zeus_single_pixel_j_jpm_fixed_jxjy_N11_N17_tiny_pilot_${RUN_TAG}"
mkdir -p "$RUN_ROOT" "$LOG_ROOT"
...
Override with `BORN_WORKERS=<n>` in `qsub -v` only when Zeus memory headroom is clear.
