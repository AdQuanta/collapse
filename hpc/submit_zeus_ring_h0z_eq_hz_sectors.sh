...
# Usage: N_PIXEL=18 J_BOND=0.37 G_COUP=0.1 TAUS=10:30:100 [WALLTIME=36:00:00] [QUEUE=zeus_new_q] [RUN_ROOT=<dir>] \
...
JOB_ID="$(qsub -q "$QUEUE" -N "ring_h0zhz_N${N_PIXEL}" -J "0-$((N_PIXEL / 2))" -l "walltime=$WALLTIME" \
  -v "RUN_ROOT=$RUN_ROOT,N_PIXEL=$N_PIXEL,J_BOND=$J_BOND,G_COUP=$G_COUP,TAUS=$TAUS" \
  "$REPO_ROOT/hpc/zeus_ring_h0z_eq_hz_sectors.pbs")"
printf '%s job_id=%s queue=%s N=%s J=%s g=%s taus=%s walltime=%s run_root=%s\n' "$(date --iso-8601=seconds)" \
  "$JOB_ID" "$QUEUE" "$N_PIXEL" "$J_BOND" "$G_COUP" "$TAUS" "$WALLTIME" "$RUN_ROOT" | tee -a "$RUN_ROOT/submitted_jobs.txt"
