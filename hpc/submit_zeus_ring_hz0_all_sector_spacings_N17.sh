...
printf '%s job_id=%s array=%s sector_shards=30 hz0_values=20 eigenvalues=512 checkpoint=each_hz0 run_root=%s\n' \
  "$(date --iso-8601=seconds)" "$JOB_ID" "$ARRAY_RANGE" "$RUN_ROOT" | tee -a "$RUN_ROOT/submitted_jobs.txt"
printf 'Expected production outputs: 30 sector COMPLETE.json files and 390 checkpoint archives.\n'
