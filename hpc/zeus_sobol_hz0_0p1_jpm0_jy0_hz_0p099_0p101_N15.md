# Zeus Sobol scan: hz in [0.099,0.101], hz0=0.1, Jpm=0, Jy=0, N=15

From the repository root on a Zeus login node, run:

```bash
bash hpc/submit_zeus_sobol_hz0_0p1_jpm0_jy0_hz_0p099_0p101_N15.sh
```
...
- `Jpm=0` exactly;
- `Jy=0` exactly;
- `hz` is sampled logarithmically in `[0.099,0.101]`;
- `N=15` and `t=1e6`.

Only `J`, `hz`, and `Jx` are sampled. `J` uses the logarithmic interval
`[1e-3,10]`, while `hz` uses `[0.099,0.101]`. `Jx` is sampled conditionally so that
