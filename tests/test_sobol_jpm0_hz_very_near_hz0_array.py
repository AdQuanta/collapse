...
from scripts.run_zeus_sobol_hz0_0p1_jpm0_jy0_hz_0p099_0p101_N15 import (
    BATCH_SIZE,
    DETECTOR_N,
    FIXED_JPM,
    FIXED_JY,
...
def test_fixed_jpm_zero_hz_very_near_hz0_sampling_is_strict_and_perturbative() -> None:
    ...
    assert min(point.hz for point in points) < 0.0992
    assert max(point.hz for point in points) > 0.1008
    assert all(
...
def test_fixed_jpm_zero_hz_very_near_hz0_array_batches_partition_all_configurations() -> None:
