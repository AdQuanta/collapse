...
from examples.run_zeus_sobol_hz0_0p1_jpm0_jy0_hz_0p09_0p11_N15 import (
    BATCH_SIZE,
    DETECTOR_N,
    FIXED_JPM,
    FIXED_JY,
...
def test_fixed_jpm_zero_hz_near_hz0_sampling_is_strict_and_perturbative() -> None:
    ...
    assert min(point.hz for point in points) < 0.092
    assert max(point.hz for point in points) > 0.108
    assert all(
...
def test_fixed_jpm_zero_hz_near_hz0_array_batches_partition_all_configurations() -> None:
    ...
    assert HZ0 == 0.1
    assert HZ_LOWER == 0.09
    assert HZ_UPPER == 0.11
